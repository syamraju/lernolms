 ...
 * re-pointing those variables at Cloudscape values re-skins the whole product
 * without touching the 235 component files.
 *
...
const RAMP = {
  light: {
    ...
    ink: ['#ffffff', '100', '200', '300', '400', '500', '600', '700', '800', '900'],
    outline: ['100', '200', '300', '400', '500', '600', '700', '800', '900', '950'],
  },
  dark: {
    surface: ['950', '900', '800', '700', '600', '500', '400', '300', '200', '100'],
    ink: ['#ffffff', '800', '700', '600', '500', '400', '300', '200', '100', '50'],
    outline: ['900', '800', '700', '600', '500', '400', '300', '200', '100', '50'],
  },
}
...
const GRAY = {
  light: {
    surface: ['#f9f9fa', '#f3f3f7', '#ebebf0', '#dedee3', '#c6c6cd', '#b4b4bb', '#8c8c94', '#656871', '#424650', '#0f141a'],
    ink: ['#dedee3', '#c6c6cd', '#b4b4bb', '#8c8c94', '#656871', '#424650', '#333843', '#232b37', '#0f141a'],
    outline: ['#dedee3', '#c6c6cd', '#b4b4bb', '#8c8c94', '#656871', '#424650', '#333843', '#232b37', '#0f141a'],
  },
  dark: {
    surface: ['#1b232d', '#232b37', '#333843', '#424650', '#656871', '#8c8c94', '#a4a4ad', '#c6c6cd', '#dedee3', '#f9f9fa'],
    ink: ['#232b37', '#333843', '#424650', '#656871', '#8c8c94', '#a4a4ad', '#c6c6cd', '#dedee3', '#f9f9fa'],
    outline: ['#333843', '#424650', '#656871', '#8c8c94', '#a4a4ad', '#c6c6cd', '#dedee3', '#ebebf0', '#f9f9fa'],
  },
}
...
const ALPHA = {
  light: ['#0f141a08', '#0f141a0d', '#0f141a17', '#0f141a24', '#0f141a3d', '#0f141a52', '#0f141a78', '#0f141aa1', '#0f141ac7', '#0f141a'],
...
const HUES = {
  red: 'red', blue: 'blue', green: 'green', amber: 'amber', violet: 'violet',
  yellow: 'yellow', orange: 'orange', teal: 'teal', cyan: 'cyan',
...
function ramps(theme) {
  ...
  for (const [fu, cds] of Object.entries(HUES)) {
    RAMP[theme].surface.forEach((s, i) => push(`--surface-${fu}-${i + 1}`, step(cds, s)))
    RAMP[theme].ink.forEach((s, i) => push(`--ink-${fu}-${i + 1}`, step(cds, s)))
    RAMP[theme].outline.forEach((s, i) => push(`--outline-${fu}-${i + 1}`, step(cds, s)))
  }
  return out
}
...
function anchors(theme) {
  ...
  return [
    ...
    ['--surface-elevation-1', dark ? '#1b232d' : t('background/container/content')],
    ['--surface-elevation-2', dark ? '#232b37' : t('background/container/content')],
    ['--surface-elevation-3', dark ? '#333843' : t('background/container/content')],
    ['--surface-alpha-elevation-1', dark ? '#1b232d' : t('background/container/content')],
    ['--surface-alpha-elevation-2', dark ? '#232b37' : t('background/container/content')],
    ['--surface-alpha-elevation-3', dark ? '#333843' : t('background/container/content')],
...
function paletteVars() {
  ...
  for (const [hue, steps] of Object.entries(P)) {
    for (const [step, value] of Object.entries(steps)) {
      out.push([`--${hue}-${step}`, value])
    }
  }
  // CourseCard's `card_gradient` values use frappe-ui's `gray` spelling.
  for (const [step, value] of Object.entries(P.grey)) out.push([`--gray-${step}`, value])
...
function cdsScales() {
  const out = []
  for (const [k, v] of Object.entries(src.spacing)) out.push([`--cds-space-${k}`, `${v}px`])
  for (const [k, v] of Object.entries(src.radius)) out.push([`--cds-radius-${k.replace(/\//g, '-')}`, `${v}px`])
  for (const [k, v] of Object.entries(src.borderWidth)) out.push([`--cds-border-${k.replace(/\//g, '-')}`, `${v}px`])
  for (const [k, v] of Object.entries(src.sizes)) out.push([`--cds-size-${k.replace(/\//g, '-')}`, `${v}px`])
  return out
}
...
function cdsSemantic(theme) {
  return Object.entries(S)
    ...
    .map(([k, v]) => [`--cds-${k.replace(/\//g, '-')}`, v[theme === 'light' ? 0 : 1]])
}
...
const css = [
  ...
  '',
  '/* ---------------------------------------------------------------- light */',
  block(':root', [...paletteVars(), ...ramps('light'), ...anchors('light'), ...elevations(), ...radii(), ...cdsScales(), ...cdsSemantic('light')]),
  '',
  '/* ----------------------------------------------------------------- dark */',
  block('[data-theme="dark"]', [...ramps('dark'), ...anchors('dark'), ...cdsSemantic('dark')]),
  '',
].join('\n')
