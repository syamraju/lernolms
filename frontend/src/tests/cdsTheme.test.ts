...
const buttonSource = read('node_modules/frappe-ui/src/components/Button/Button.vue')
...
describe('CDS token layer', () => {
	...
	it('declares every --cds-* token the component layer references', () => {
		...
		const declared = new Set([
			...
			// The typography layer declares the two font stacks (--cds-font-sans,
			// --cds-font-mono); the component layer uses the mono one for code.
			...[...typographyCss.matchAll(/(--cds-[\w-]+)\s*:/g)].map((m) => m[1]),
			...
			...[...componentsCss.matchAll(/^\s*(--cds-[\w-]+)\s*:/gm)].map((m) => m[1]),
		])
...
describe('CDS typography layer', () => {
	...
	it('uses Cloudscape headings weight and tracking', () => {
		expect(typographyCss).toContain('font-weight: 700')
		// CDS heading tracking is negative; frappe-ui's Inter correction was
		// +0.02em and must be gone.
		expect(typographyCss).not.toContain('0.02em')
		expect(typographyCss).toContain('letter-spacing: -0.48px')
		expect(typographyCss).toContain('Open Sans Variable')
	})

	// Setting font-family on div/span/p beats inheritance (an element selector
	// outranks an inherited value), which silently overrode the DM Sans the
	// student shell sets on `.learno`. The family belongs on `html` alone.
	it('sets the font family on html only, so subtrees can override it', () => {
		const familyRules = [...typographyCss.matchAll(/([^{}]+)\{[^{}]*font-family[^{}]*\}/g)]
			.map((m) => m[1].trim())
			.filter((sel) => !sel.startsWith('/*') && !sel.includes(':root'))
		for (const sel of familyRules) {
			expect(sel).not.toMatch(/\bdiv\b|\bspan\b|\bp\b,/)
		}
	})
})

describe('component-layer scoping', () => {
	// frappe-ui puts data-slot="control" on checkboxes as well as switches, and
	// both carry data-state="unchecked" — so a toggle rule keyed on the slot
	// painted every unchecked checkbox in every list view dark grey. The ARIA
	// role is the only thing that separates them.
	it('scopes the toggle rules to [role=switch], not the shared control slot', () => {
		const toggleBlock = componentsCss.slice(
			componentsCss.indexOf('toggle --'),
			componentsCss.indexOf('progress bar --'),
		)
		expect(toggleBlock).toContain("[role='switch']")
		expect(toggleBlock).not.toContain("[data-slot='control']")
	})

	// Form/Label (12px bold) is the caption above a control. A checkbox's inline
	// label is Form/Default, and frappe-ui gives both the same data-slot.
	it('exempts checkbox and radio labels from the bold field-label style', () => {
		expect(componentsCss).toContain(
			":has(> input[type='checkbox']) > [data-slot='label']",
		)
		expect(componentsCss).toContain(
			":has(> input[type='radio']) > [data-slot='label']",
		)
	})
})
