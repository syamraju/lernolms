	...
	<div class="flex h-full min-h-0 flex-col">
		...
		>
			...
			<p class="mt-1 text-[13px] text-[var(--learno-ink-muted)]">
				{{
					__(
						'Message your batch or anyone in it, and call them without leaving the page.'
					)
				}}
			</p>
		</header>
		...
		<div class="flex min-h-0 flex-1">
			...
			>
				<div class="mb-2 flex items-center justify-between px-2">
					<h2
						class="text-[11px] font-semibold uppercase tracking-wide text-[var(--learno-ink-subtle)]"
					>
						{{ __('Direct messages') }}
					</h2>
					<button
				...
				<template v-if="selected">
					...
					>
						<div class="min-w-0 flex-1">
							<p
								class="truncate text-[15px] font-semibold text-[var(--learno-ink-strong)]"
							>
								{{ selectedTitle }}
							</p>
					...
					>
						<p
							class="max-w-sm text-center text-[13px] text-[var(--learno-ink-muted)]"
						>
							{{
								__(
									'This is a live class room. Use the call buttons above to join.'
								)
							}}
						</p>
					</div>
...
const conversations = createResource({
	url: 'lms.lms.direct_message.get_conversations',
	auto: true,
})
...
const directThreads = computed(() => {
	...
	return [
		...fetched,
		...pendingThreads.value.filter((t) => !known.has(t.conversation)),
	]
})
...
const selectedTitle = computed(() => {
	...
	if (isDirect.value) {
		const thread = directThreads.value.find(
			(t: any) => t.conversation === selected.value
		)
...
async function refreshActive() {
	...
	activeCalls.value = await call('lms.lms.huddle.get_active', {
		conversations: ids,
	}).catch(() => activeCalls.value)
}
...
watch([myBatches, directThreads], () => {
	if (selected.value) return
	const first =
		directThreads.value[0]?.conversation || myBatches.value[0]?.conversation
...
async function openLinkedConversation() {
	...
	try {
		linked.value = await call('lms.lms.direct_message.get_thread', {
			conversation: target,
		})
	} catch {
